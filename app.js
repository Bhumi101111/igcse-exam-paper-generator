...
function questionDedupeKey(item){const template=normalizeQuestionTemplate(item.q[0]);return `${item.chapter||"general"}::${template}`;}
function questionKeyLimit(){return 1;}
function exactQuestionKey(item){return `${item.chapter||"general"}::${normalizeQuestionText(item.q[0])}`;}
function fillSection(source,target,type,startIndex,usedCounts,exactSeen){if(!target||!source.length)return [];const result=[],counts=usedCounts||new Map(),seen=exactSeen||new Set(),limit=questionKeyLimit();let total=0;const prepared=orderByDifficulty(source),offset=prepared.length?startIndex%prepared.length:0,ordered=[...prepared.slice(offset),...prepared.slice(0,offset)];const take=item=>{const exactKey=exactQuestionKey(item);if(seen.has(exactKey)||total>=target)return false;const q=item.q,questionMarks=Math.min(q[1],target-total);result.push({text:q[0],marks:questionMarks,answer:q[2],diagram:q[3]||"",type,chapter:item.chapter});seen.add(exactKey);total+=questionMarks;return true;};
// Pass 1: at most one question per template so near-identical wording never repeats.
for(const item of ordered){if(total>=target)break;const key=questionDedupeKey(item),used=counts.get(key)||0;if(used>=limit)continue;if(take(item))counts.set(key,used+1);}
// Pass 2: top up to the requested marks using distinct numeric variants (never the exact same text).
for(const item of ordered){if(total>=target)break;take(item);}
// Pass 3: last resort — once all distinct content is used, cycle through (spaced) so the paper always reaches the requested marks.
let guard=0;while(total<target&&ordered.length&&guard<5000){for(const item of ordered){if(total>=target)break;const q=item.q,questionMarks=Math.min(q[1],target-total);result.push({text:q[0],marks:questionMarks,answer:q[2],diagram:q[3]||"",type,chapter:item.chapter});total+=questionMarks;}guard++;}
return result;}
function buildQuestions(){const sources=questionSources(),split=getSplit(),usedCounts=new Map(),exactSeen=new Set();state.questions=hasSplit()?[...fillSection(sources.Theory,split.theory,"Theory",state.seed,usedCounts,exactSeen),...fillSection(sources.Numerical,split.numerical,"Numerical",state.seed,usedCounts,exactSeen)]:fillSection(sources.Questions,+$("marks").value,"Questions",state.seed,usedCounts,exactSeen);state.seed++;}
